...
#ifdef USE_CRAB_INTRINSICS
extern int32_t __CRAB_intrinsic_red(int64_t);
#endif
...
static int32_t mul_red(int32_t x, int32_t y) {
  ...
  z = (int64_t) x * y;
#ifdef SAFE  
  clam_assert(-8796042698752 <= z);
  clam_assert(z <= 8796093026303);
#else  
  clam_assert(-62029812 <= z);
  clam_assert(z <= 14813490);
#endif   
#ifdef USE_CRAB_INTRINSICS
  return __CRAB_intrinsic_red(z);
#else
  x = z & 4095;
  y = z >> 12;
  return 3 * x - y;
...
void ntt_red_ct_rev2std(int32_t *a, uint32_t n, const int16_t *p) {
  ...
  #ifdef UNROLLING_NTT_1024
  #pragma clang loop unroll_count(10)
  #endif
  #ifdef UNROLLING_NTT_16
  ...
  for (t=1; t<n; t <<= 1) {
    ...
    #ifdef UNROLLING_NTT_1024
    #pragma clang loop unroll_count(1023)
    #endif
    #ifdef UNROLLING_NTT_16
    //#pragma clang loop unroll_count(16)    
    ...
    for (j=1; j<t; j++) {
      w = p[t+j];   // w_t^j/3
      #ifdef UNROLLING_NTT_1024
      #pragma clang loop unroll_count(4097)
      #endif
      #ifdef UNROLLING_NTT_16
      //#pragma clang loop unroll_count(16)      
